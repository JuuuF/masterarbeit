...
\subsection*{1. Ist es möglich, eine Pipeline zur automatischen Erstellung annotierter, realistischer und variabler Bilder von Dartscheiben zu erstellen?}

Diese Fragestellung bezieht sich inhaltlich auf die Ausarbeitung der synthetischen Datenerstellung, welche den ersten Teil dieser Arbeit ausmacht. Diese Frage umfasst in erster Instanz die Umsetzbarkeit der automatisierten Erstellung von Bildern mit Dartscheiben. Anschließend ist diese Datenerstellung mit einer Randomisierung von Parametern verbunden und anschließender Annotation der Daten hinsichtlich der Position der Dartpfeilspitzen in den Bildern. Zuletzt gilt es, diese Datenerstellung in eine automatisierte Pipeline einzubinden, mit der eine autonome Erstellung einer Vielzahl unterschiedlicher Daten ermöglicht wird.

Diese Daten unterliegen zusätzlich der Bedingung, ein möglichst realistisches Erscheinungsbild aufzuweisen und eine große Spanne unterschiedlicher Aussehen und Beleuchtungen abzudecken, sodass die Wahrscheinlichkeit der Datenverzerrung durch einseitige Darstellung minimiert wird.
...
\subsection*{3. Wie zuverlässig agiert ein durch \ac{ood}-Training mit synthetischen Daten trainiertes neuronales Netz auf echten Daten?}

Die für die Beantwortung der ersten Forschungsfrage erstellten Daten dienen dem Training eines neuronalen Netzes. Durch die Verwendung synthetischer Daten als Datenlage für das Training sind Unterscheide zwischen Trainings- und Inferenzdaten zu erwarten. Der Grad des Unterschieds unterliegt der Qualität der Datenerstellung, kann jedoch als nicht unerheblich eingestuft werden. Diese Forschungsfrage untersucht die Fähigkeit eines neuronalen Netzes, zuverlässige Vorhersagen auf realen Daten trotz dieser Differenzen zu erbringen.

\subsection*{4. Wie kann sich das erarbeitete Gesamtsystem dieser Arbeit im Vergleich zu DeepDarts hinsichtlich Performance und Genauigkeit behaupten?}
