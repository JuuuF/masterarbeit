...
\begin{center}
    \begin{tikzpicture}
        \begin{axis}[
            ...
            \addplot[name path = upper, thick, data_primary]{0.21 * x + 28.5};
            \addplot[name path = lower, thick, data_secondary]{0.21 * x + 7 . 5};
            \addplot[color=data_secondary, opacity=0.5] fill between[of = upper and lower];
            \legend{Obergrenze, Untergrenze, Mögliche Brennwenten};
        \end{axis}
    \end{tikzpicture}
    \captionof{figure}{Abhängigkeit der Brennweiten von dem Abstand zur Dartscheibe.}
